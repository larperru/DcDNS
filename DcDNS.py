...
DCDNS_PAYLOAD = (
    ...
    "        var MULLVAD_DOH_FALLBACK = 'https://adblock.dns.mullvad.net/dns-query';\n"
    ...
    "            safeSwitch('enable-features', 'DnsOverHttps:Fallback/false/Templates/https%3A%2F%2Fdns.mullvad.net%2Fdns-query%20https%3A%2F%2Fadblock.dns.mullvad.net%2Fdns-query');\n"
    "        }\n"
    "        var DCDNS_DOH_SERVERS = [MULLVAD_DOH, MULLVAD_DOH_FALLBACK];\n"
...
RULE 1 - Encrypted DNS (DNS-over-HTTPS)
  ...
  through Mullvad's encrypted DoH resolvers:
...
safeBind('btn-discord-invite', 'click', function() {
  try {
    if (window.pywebview && window.pywebview.api && typeof window.pywebview.api.open_discord === 'function') {
      window.pywebview.api.open_discord();
    }
  } catch (err) { console.error('[DcDNS] open_discord failed:', err); }
});
